...
import { getCustomerListService, getDashboardInfoService } from './admin.service.js';

export const getDashboardInfoController = async (req, res) => {
  try {
    const data = await getDashboardInfoService(req.query);
    generateResponse(res, 200, true, 'Dashboard info fetched successfully', data);
  } catch (error) {
    console.log(error)
    generateResponse(res, 500, false, 'Failed to fetch dashboard info', null);
  }
};
...
export const getCustomerListController = async (req, res) => {
  try {
    const { page = 1, limit = 10 } = req.query;
    const data = await getCustomerListService({ page, limit });
    generateResponse(res, 200, true, 'Customer list fetched successfully', data);
  } catch (error) {
    console.log(error)
    generateResponse(res, 500, false, 'Failed to fetch customer list', null);
  }
};
